...
const Loading = ({ size = 'medium', text }: LoadingProps) => {
  ...
  const defaultText = text || tl({ en: 'Loading...', th: 'กำลังโหลด...' })
  const sizeClasses = {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-10 bg-neo-pink border-4 border-black">
      ...
      <h3 className="text-2xl font-black mb-2 uppercase">
        {tl({ en: 'Oops! Something went wrong', th: 'โอ้! มีบางอย่างผิดพลาด' })}
      </h3>
      <p className="font-mono text-gray-700 mb-6">
        {error || tl({ en: 'Unknown error occurred', th: 'เกิดข้อผิดพลาดที่ไม่ทราบ' })}
      </p>
      {onRetry && (
        <button
          onClick={onRetry}
        ...
        >
          {tl({ en: 'Try Again', th: 'ลองอีกครั้ง' })}
        </button>
      )}
    </div>
  )
