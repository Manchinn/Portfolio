  ...
  return (
    <main className="min-h-screen bg-[#f6f8fb] text-slate-950">
      <div className="mx-auto grid max-w-[1800px] gap-5 px-4 py-5 lg:px-6">
        <section className={`border border-slate-200 bg-white p-6 ${styles.glow} sm:p-8`}>
          <div className="grid gap-8 xl:grid-cols-[1.28fr_0.82fr] xl:items-end">
            <div>
              <Link href="/demos" className="inline-flex items-center border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-black uppercase tracking-[0.12em] text-slate-600 transition hover:border-slate-300 hover:bg-white">
                Back to demos
              </Link>
              <div className="mt-8">
            ...
            <div className="grid gap-3">
              ...
              <div className="grid gap-3 sm:grid-cols-3 xl:grid-cols-1">
                {metrics.map((metric) => (
                  <div key={metric.label} className="border border-slate-200 bg-white p-4">
        ...
        <section className="grid gap-5 xl:grid-cols-[0.72fr_1.55fr]">
          <aside className="grid gap-5">
            <div className="border border-slate-200 bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.06)]">
              ...
              <div className="space-y-3">
                {workflow.map((step, index) => (
                  <article key={step.label} className="border border-slate-200 bg-slate-50 p-4">
            ...
            <div className="border border-slate-200 bg-white p-6 shadow-[0_18px_50px_rgba(15,23,42,0.06)]">
              ...
              <div className="grid gap-3">
                {safeguards.map((item) => (
                  <div key={item} className="flex gap-3 border border-slate-200 bg-white p-3 text-sm font-semibold text-slate-600">
          ...
          <div className="grid gap-5">
            <section className="border border-slate-200 bg-[#111827] p-5 text-white shadow-[0_18px_50px_rgba(15,23,42,0.14)]">
                  ...
                  <div className="space-y-3">
                    {assistantQuestions.map((item) => (
                      <button
                        ...
                        className={`w-full border px-4 py-3 text-left text-sm font-black transition ${
              ...
              {table && (
                <div className="overflow-hidden border border-slate-700 bg-slate-950">
                  ...
                  {table.rows.map((row, index) => (
                    <div key={index} className="grid border-t border-slate-700 px-4 py-3 font-mono text-sm text-slate-200" style={{ gridTemplateColumns: `repeat(${table.columns.length}, minmax(0, 1fr))` }}>
              ...
              <div className={`border ${styles.border} ${styles.soft} p-6`}>
                ...
                <div className="grid gap-3">
                  {[Workflow, Activity, Bell, FileText, Network, Bot].map((ActionIcon, index) => (
                    <div key={index} className="flex items-center gap-3 border border-white/70 bg-white p-3 text-sm font-bold text-slate-700">
                ...
                <Link href={cta.href} className={`mt-5 inline-flex w-full items-center justify-center ${styles.bg} px-5 py-3 text-xs font-black uppercase tracking-[0.12em] text-white transition hover:opacity-90`}>
                  {cta.label}
