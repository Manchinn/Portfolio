import type { MetadataRoute } from 'next'

export default function sitemap(): MetadataRoute.Sitemap {
  return [
    { url: 'https://www.chinnakrit.dev', lastModified: new Date(), priority: 1 },
    { url: 'https://www.chinnakrit.dev/#work', lastModified: new Date(), priority: 0.8 },
    { url: 'https://www.chinnakrit.dev/#stack', lastModified: new Date(), priority: 0.8 },
    { url: 'https://www.chinnakrit.dev/#contact', lastModified: new Date(), priority: 0.5 },
    { url: 'https://www.chinnakrit.dev/work-with-me', lastModified: new Date(), priority: 0.8 },
    { url: 'https://www.chinnakrit.dev/saas', lastModified: new Date(), priority: 0.7 },
    { url: 'https://www.chinnakrit.dev/prompts', lastModified: new Date(), priority: 0.7 },
  ]
}
