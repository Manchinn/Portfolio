    ...
    <main className="min-h-dvh bg-saas-bg text-saas-ink">
      <header className="border-b border-saas-line">
        <div className="mx-auto max-w-[1100px] px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
          ...
          <p className="mt-10 text-xs font-semibold uppercase text-saas-accent">{c.proof}</p>
          <div className="mt-4 flex flex-wrap items-baseline gap-4">
            <span className="text-sm font-semibold text-saas-muted">{project.category}</span>
            <span className="text-sm text-saas-muted">{project.date}</span>
          </div>
          <h1 className="mt-4 max-w-[20ch] break-words text-4xl font-semibold leading-tight sm:text-5xl">{project.title}</h1>
          <p className="mt-6 max-w-3xl text-lg leading-8 text-saas-muted">{project.description}</p>
        </div>
      </header>
      ...
      <section className="mx-auto max-w-[1100px] px-4 py-14 sm:px-6 sm:py-18 lg:px-8">
        <dl className="grid gap-8 lg:grid-cols-3">
          {caseStudyRows.map((row) => (
            <div key={row.label} className="border-t border-saas-line pt-6">
              <dt className="text-xs font-semibold uppercase text-saas-accent">{row.label}</dt>
              <dd className="mt-3 text-base leading-7 text-saas-muted">{row.value}</dd>
            </div>
          ))}
        </dl>

        <div className="mt-14 grid gap-12 border-t border-saas-line pt-10 lg:grid-cols-2">
          <section>
            <h2 className="text-2xl font-semibold">{c.delivery}</h2>
            <ul className="mt-6 grid gap-4">
              {project.highlights.map((highlight) => (
                <li key={highlight} className="flex gap-3 text-base leading-7 text-saas-muted">
                  <CheckCircle2 className="mt-1 size-5 shrink-0 text-saas-accent" />
                  <span>{highlight}</span>
                </li>
              ))}
            </ul>
          ...
          <section>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              {project.tech.map((item) => (
                <span key={item} className="rounded-[6px] border border-saas-line bg-saas-surface px-3 py-2 text-sm font-medium text-saas-muted">
                  {item}
                </span>
              ))}
            </div>
        ...
        <section id="demo" className="mt-14 scroll-mt-24 border-y border-saas-line py-10">
          <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
            <div className="min-w-0">
              <p className="text-xs font-semibold uppercase text-saas-accent">{c.demoEyebrow}</p>
              <h2 className="mt-3 max-w-2xl text-3xl font-semibold leading-tight">{c.demoTitle}</h2>
              <p className="mt-4 max-w-2xl text-base leading-7 text-saas-muted">{c.demoBody}</p>
            </div>
            <div>
              <p className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase text-saas-muted">
                <ListFilter className="size-4" />
                {c.filterLabel}
              </p>
              <div className="inline-flex max-w-full flex-wrap rounded-[6px] border border-saas-line bg-saas-surface p-1" role="group" aria-label={c.filterLabel}>
                {filters.map((item) => (
                  <button
                    key={item.value}
                    type="button"
                    ...
                    className={`min-h-10 rounded-[4px] px-3 py-2 text-sm font-semibold transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-saas-accent ${
                      filter === item.value
          ...
          <p className="mt-8 text-sm font-medium text-saas-muted" aria-live="polite">
            {visibleRecords.length} {c.records}
          </p>
          <table className="mt-3 w-full border-y border-saas-line text-left">
            <thead className="hidden sm:table-header-group">
              <tr className="border-b border-saas-line text-xs font-semibold uppercase text-saas-muted">
            ...
            <tbody>
              {visibleRecords.map((record) => (
                <tr key={record.id} className="block border-b border-saas-line py-5 last:border-b-0 sm:table-row sm:py-0">
                  <th scope="row" className="block py-1 font-mono text-sm font-semibold text-saas-ink sm:table-cell sm:w-[90px] sm:py-5 sm:pr-4">
                    <span className="text-xs font-semibold uppercase text-saas-muted sm:hidden">{c.record}: </span>
                    {record.id}
                  </th>
                  <td className="block min-w-0 py-1 sm:table-cell sm:py-5 sm:pr-4">
                    <span className="text-xs font-semibold uppercase text-saas-muted sm:hidden">{c.activity}</span>
                    <p className="break-words text-sm font-medium text-saas-ink">{record.activity}</p>
                  </td>
                  <td className="block py-1 sm:table-cell sm:w-[130px] sm:py-5 sm:pr-4">
                    <span className="text-xs font-semibold uppercase text-saas-muted sm:hidden">{c.status}</span>
                  ...
                  <td className="block py-1 text-sm text-saas-muted sm:table-cell sm:w-[110px] sm:py-5">
        ...
        <div className="mt-14 grid gap-6 border-y border-saas-line py-9 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
